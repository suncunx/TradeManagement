...
public class GoodsServlet extends BaseServlet {
	...
	public void save(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
		...
		System.out.println(userId + name + unit + inUnitPrice + outUnitPrice + supplierId);
		// todo 同时获取表单和文件
		try {
	...
	private void detectSaveFile(HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	private void processSaveUploadedFile(FileItem item, HttpServletResponse response) throws Exception {
		...
		File uploadedFile = new File(Constant.FILE_PATH + time + ".jpg");
		item.write(uploadedFile);
		dbUtil.saveGoods(con, userId, name, unit, inUnitPrice, outUnitPrice, time + ".jpg", supplierId);
	...
	public void update(HttpServletRequest request, HttpServletResponse response) throws UnsupportedEncodingException {
		...
		System.out.println(goodsId + name + unit + inUnitPrice + outUnitPrice + supplierId);
	...
	private void detectUpdateFile(HttpServletRequest request, HttpServletResponse response) throws Exception {
		// Create a factory for disk-based file items
		DiskFileItemFactory factory = new DiskFileItemFactory();
	...
	private void processUpdateUploadedFile(FileItem item, HttpServletResponse response) throws Exception {
		String fieldName = item.getFieldName();
		String fileName = item.getName();
		String contentType = item.getContentType();
		...
		File uploadedFile = new File(Constant.FILE_PATH + time + ".jpg");
		item.write(uploadedFile);
		dbUtil.updateGoods(con, userId, goodsId, name, unit, inUnitPrice, outUnitPrice, time + ".jpg", supplierId);
		doResponse(response, getResult(200, "修改成功", "修改成功"));
	}
